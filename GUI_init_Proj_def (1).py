...
class GalvoWorker_MDA(QObject):  
    ...
    def generate_voltage_sequences(self):
        ...
        Galvo2_Enable = self.voltage_control_widget.get_Galvo2_Enable()
        galvo2_Value = self.voltage_control_widget.get_galvo2_Value()
        # Calculate the new min and max voltages using the factor
        min_voltage_new, max_voltage_new = self.increase_range()
            ...
            for frame_index in range(num_frames):
                for slice_index in range(num_slices):
                    for exposure_time in exposure_times:
                        duration_ms = exposure_time + 22.937  # Calculate duration based on exposure_time
...
class VoltageControlWidget(QWidget): 
    ...
    def init_ui(self):  # Method to initialize the UI
        ...
        self.btn_toggle_slider = QPushButton('No 2nd Galvo')
        ...
        self.label_factor = QLabel('Factor : x 1')
